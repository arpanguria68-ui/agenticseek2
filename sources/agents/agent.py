...
class Agent:
    ...
    def extract_reasoning_text(self, text: str) -> None:
        ...
        if start_idx == -1:  # No thinking block
            return None

        if end_idx == -1:
            # Incomplete thinking block, return everything after start_tag
            return text[start_idx:]

        return text[start_idx : end_idx + 8]
    ...
    def execute_modules(self, answer: str) -> Tuple[bool, str]:
        ...
        for name, tool in self.tools.items():
            ...
            if blocks != None:
                pretty_print(
                    f"Executing {len(blocks)} {name} blocks...", color="status"
                )
                for block in blocks:
