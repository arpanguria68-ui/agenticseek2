...
class Interaction:
    ...
    def load_last_session(self):
        """Recover the last session for all agents including planner."""
        for agent in self.agents:
    ...
    async def think(self) -> bool:
        ...
        self.last_answer, self.last_reasoning = await agent.process(
            self.last_query, self.speech
        )
        self.is_generating = False
    ...
    def speak_answer(self) -> None:
        ...
        if self.tts_enabled and self.last_answer and self.speech:
            ...
            thread = threading.Thread(
                target=speak_in_thread, args=(self.speech, self.last_answer)
            )
            thread.start()
