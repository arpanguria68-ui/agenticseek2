...
class Memory:
    ...
    def get_ideal_ctx(self, model_name: str) -> int | None:
        ...
        self.logger.info(
            f"Estimated context size for {model_name}: {context_size} tokens."
        )
        return context_size
    ...
    def save_memory(self, agent_type: str = "casual_agent") -> None:
        ...
        json_memory = json.dumps(self.memory, indent=2)
        with open(path, "w") as f:
            self.logger.info(f"Saved memory json at {path}")
            f.write(json_memory)

        # Also save as 'latest.json' for reliable recovery
        latest_path = os.path.join(save_path, "latest.json")
        with open(latest_path, "w") as f:
            f.write(json_memory)
            self.logger.info(f"Updated latest session at {latest_path}")

    def find_last_session_path(self, path) -> str:
        """Find the last session path."""
        # First check for latest.json
        latest_file = os.path.join(path, "latest.json")
        if os.path.exists(latest_file):
            self.logger.info(f"Using latest session file: {latest_file}")
            return "latest.json"

        # Fallback to timestamp-based files
        saved_sessions = []
    ...
    def load_memory(self, agent_type: str = "casual_agent") -> None:
        ...
        if (
            not loaded_memory
            or not isinstance(loaded_memory, list)
            or len(loaded_memory) == 0
        ):
            pretty_print("Session file was empty or invalid.", color="warning")
            return

        self.memory = loaded_memory

        # Remove trailing user message if present (incomplete conversation)
        if len(self.memory) > 0 and self.memory[-1].get("role") == "user":
            self.memory.pop()

        self.logger.info(f"Loaded {len(self.memory)} messages from session")
        pretty_print(
            f"Session recovered: {len(self.memory)} messages loaded", color="success"
        )
    ...
    def push(self, role: str, content: str) -> int:
        ...
        if ideal_ctx is not None:
            if self.memory_compression and len(content) > ideal_ctx * 1.5:
                self.logger.info(
                    f"Compressing memory: Content {len(content)} > {ideal_ctx} model context."
                )
                self.compress()
    ...
    def compress_text_to_max_ctx(self, text) -> str:
        ...
        while len(text) > ideal_ctx:
            self.logger.info(
                f"Compressing text: {len(text)} > {ideal_ctx} model context."
            )
